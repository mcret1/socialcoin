...
{
    /* current difficulty formula, socialcoin - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
...
unsigned int GetLegacyNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock, bool fProofOfStake)
{
    ...
    // socialcoin coin: target change every block
    // socialcoin coin: retarget with exponential moving toward target spacing
    uint256 bnNew;
    bnNew.SetCompact(pindexPrev->nBits);

    if (pindexLast->nHeight+1 >= nHardForkBlock)
    {
        int64_t socialcoinerval = nTargetTimespan / nTargetSpacing;
        bnNew *= ((socialcoinerval - 1) * nTargetSpacing + nActualSpacing + nActualSpacing);
        bnNew /= ((socialcoinerval + 1) * nTargetSpacing);
    }
    else
    {
        int64_t socialcoinerval = nTargetTimespan / nTargetSpacingOld;
        bnNew *= ((socialcoinerval - 1) * nTargetSpacingOld + nActualSpacing + nActualSpacing);
        bnNew /= ((socialcoinerval + 1) * nTargetSpacingOld);
    }
