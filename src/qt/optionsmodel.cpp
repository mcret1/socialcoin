...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::socialcoin);
    ...
    if (!settings.contains("nAnonymizesocialcoinAmount"))
        settings.setValue("nAnonymizesocialcoinAmount", 1000);

    nAnonymizesocialcoinAmount = settings.value("nAnonymizesocialcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizesocialcoinAmount"))
        SoftSetArg("-anonymizesocialcoinamount", settings.value("nAnonymizesocialcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in socialcoin.cpp during shotdown to also remove the window positions
...
//            return settings.value("fShowMasternodesTab");
        ...
        case AnonymizesocialcoinAmount:
            return QVariant(nAnonymizesocialcoinAmount);
        case Listen:
...
//            break;
        ...
        case AnonymizesocialcoinAmount:
            nAnonymizesocialcoinAmount = value.toInt();
            settings.setValue("nAnonymizesocialcoinAmount", nAnonymizesocialcoinAmount);
            emit anonymizesocialcoinAmountChanged(nAnonymizesocialcoinAmount);
            break;
        case CoinControlFeatures:
