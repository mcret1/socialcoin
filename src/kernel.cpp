...
// Get time weight
int64_t GetWeight(int64_t socialcoinervalBeginning, int64_t socialcoinervalEnd)
{
    return min(socialcoinervalEnd - socialcoinervalBeginning - nStakeMinAge, (int64_t)nStakeMaxAge);
}
...
static int64_t GetStakeModifierSelectiosocialcoinervalSection(int nSection)
{
...
static int64_t GetStakeModifierSelectiosocialcoinerval()
{
    int64_t nSelectiosocialcoinerval = 0;
    for (int nSection = 0; nSection < 64; nSection++) {
        nSelectiosocialcoinerval += GetStakeModifierSelectiosocialcoinervalSection(nSection);
    }
    return nSelectiosocialcoinerval;
}

// select a block from the candidate blocks in vSortedByTimestamp, excluding
// already selected blocks in vSelectedBlocks, and with timestamp up to
// nSelectiosocialcoinervalStop.
static bool SelectBlockFromCandidates(
    vector<pair<int64_t, uint256> >& vSortedByTimestamp,
    map<uint256, const CBlockIndex*>& mapSelectedBlocks,
    int64_t nSelectiosocialcoinervalStop,
    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = (const CBlockIndex*)0;
    BOOST_FOREACH (const PAIRTYPE(int64_t, uint256) & item, vSortedByTimestamp) {
        ...
        if (fSelected && pindex->GetBlockTime() > nSelectiosocialcoinervalStop)
            break;
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    int64_t nSelectiosocialcoinerval = GetStakeModifierSelectiosocialcoinerval();
    int64_t nSelectiosocialcoinervalStart = (pindexPrev->GetBlockTime() / getIntervalVersion(fTestNet)) * getIntervalVersion(fTestNet) - nSelectiosocialcoinerval;
    const CBlockIndex* pindex = pindexPrev;

    while (pindex && pindex->GetBlockTime() >= nSelectiosocialcoinervalStart) {
    ...
    int64_t nSelectiosocialcoinervalStop = nSelectiosocialcoinervalStart;
    map<uint256, const CBlockIndex*> mapSelectedBlocks;
    for (int nRound = 0; nRound < min(64, (int)vSortedByTimestamp.size()); nRound++) {
        // add an interval section to the current selection round
        nSelectiosocialcoinervalStop += GetStakeModifierSelectiosocialcoinervalSection(nRound);

        // select a block from the candidates of current round
        if (!SelectBlockFromCandidates(vSortedByTimestamp, mapSelectedBlocks, nSelectiosocialcoinervalStop, nStakeModifier, &pindex))
        ...
        if (fDebug || GetBoolArg("-printstakemodifier", false))
            LogPrintf("ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d\n",
                nRound, DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nSelectiosocialcoinervalStop).c_str(), pindex->nHeight, pindex->GetStakeEntropyBit());
    }
...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    int64_t nStakeModifierSelectiosocialcoinerval = GetStakeModifierSelectiosocialcoinerval();
    const CBlockIndex* pindex = pindexFrom;
    CBlockIndex* pindexNext = chainActive[pindexFrom->nHeight + 1];

    // loop to find the stake modifier later by a selection interval
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectiosocialcoinerval) {
        if (!pindexNext) {
            // Should never happen
